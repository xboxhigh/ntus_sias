$(function () {
    ...
    $(".pagination > .page-item > .page-link.next").click(function () {
        ...
        $("body").find('div[id^="results-area"]').each(function () {
            var re = /-/gi;
            var key = $(this).attr('id');

            if (paramValidation(key)) {
                var input_names = key.substring(0, key.length - 2);
...
function _init() {
    ...
    $("#diagnosis-code-other").prop("checked", false);
...
function fillOutForm(data_obj) {
    ...
    $.each(data_obj, function (key, value) {
        ...
        if (key_of_value.includes("results-area")) {

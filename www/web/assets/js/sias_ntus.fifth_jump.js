$(function () {
    ...
    $(".pagination > .page-item > .btn.btn-warning").click(function () {
        ...
        if (isModify != 0) {
            ...
            var ubi_id = $('#line-id-with-ubi-id').find("option:selected").val();
            /** parameters for ajax process */
            var api_params = {
                baseUrl: "http://" + window.location.hostname + ":8100/api/web/",
                api_ver: "v1",
    ...
    $('#line-id-with-ubi-id').on('change', function () {
        if ($(this).val() != 0) {
            $('#mh-id').val(0);
            _init();
            var ubi_id = $(this).find("option:selected").val();
            $("#hidden-form").show();
    ...
    $('#mh-id').on('change', function () {
        if ($(this).val() != 0) {
            $('#line-id-with-ubi-id').val(0);
            _init();
            var mh_id = $(this).find("option:selected").val();
            $("#hidden-form").show();
            $(".pagination-wrap").show();

            if (paramValidation(mh_id)) {
                /** parameters for ajax process */
                var api_params = {
                    baseUrl: "http://" + window.location.hostname + ":8100/api/web/",
                    api_ver: "v1",
...
function fillOutOption(data_obj) {
    var option_html = ["<option value='0'>...</option>"];
    var option_html2 = ["<option value='0'>...</option>"];
    $.each(data_obj, function (key, value) {
        var ubi_id = value['ubi_id'];
        var text = value['line_id_with_ubi_id'];
        var mh_ids = value['mh_id'];
        var content = `<option value='${ubi_id}'>${text}</option>`;

        option_html.push(content);
        $.each(mh_ids, function (key, value) {
            var content2 = `<option value='${value}'>[${ubi_id}] - ${value}</option>`;
            option_html2.push(content2);
        });
    });
    $("#line-id-with-ubi-id").html(option_html.join(''));
    $("#mh-id").html(option_html2.join(''));
